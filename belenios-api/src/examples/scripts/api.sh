...
belenios-tool credgen --uuid $UUID --group src/files/groups/default.json --file $DIR/voters.txt --dir $DIR
mv $DIR/*.pubcreds $DIR/public_creds.txt
mv $DIR/*.privcreds $DIR/private_creds.txt
belenios-tool trustee-keygen --group src/files/groups/default.json
...
belenios-tool mkelection --uuid $UUID --group src/files/groups/default.json --template src/files/templates/questions.json --dir $DIR
#APP: QUESTION EDITOR ()
...
echo belenios-tool compute-voters --dir $DIR --privcred $DIR/private_creds.txt
